...
The technical reference for changing code in this project. **Read this in full before you make any change to RunOS's code, schema, or behaviour.** The coach persona and product context live in `CLAUDE.md`; this document is purely the engineering contract.
...
**v1.0 + v1.1 + v1.2 + v1.3 + v1.4 + v1.5 shipped, all on `main` at https://github.com/rossheadington/RunOS (public, code-only).**
...
| v1.3 | 14 | First-run setup wizard (`runos setup`) |
| v1.4 | 15 | Weight tracker (`weight.md`) with EWMA trend, kg/lb normalisation, recovery report integration |
| v1.5 | 16 | Nutrition tracker (`food.md`) with two-format parser, `runos analyze nutrition` standalone report, recovery report integration |

**Operational model (post-simplification, 2026-05-28):**

- Hourly: `runos sync --notify-on-failure` via launchd (`com.runos.hourly-sync.plist`). Strava + Garmin → raw. Telegram message only on failure; silent on success.
- Always-on: `runos bot run` via launchd (`com.runos.telegram-bot.plist`, KeepAlive=true). Listens for owner-only messages, routes to a Claude Code session.
- **No scheduled reports.** Reports are generated on-demand by asking the bot ("how am I doing for recovery?") — the agent runs `runos analyze <type>` and returns the rendered markdown. The old daily plist is deprecated; remove with `launchctl unload -w ~/Library/LaunchAgents/com.runos.daily.plist && rm ~/Library/LaunchAgents/com.runos.daily.plist`.
...
Two-layer raw → structured store. Connectors write **only** to `raw_response`. Pure transforms read raw and write structured tables. The structured layer is rebuildable from raw with **zero network** via `runos rederive`.
...
└────────────────────┘    └────────────────────┘    │ daily_summary      │
   ...
   ┌─────────────────────────────────────────────────┐
   │ hourly: runos sync --notify-on-failure          │
   ...
   ┌─────────────────────────────────────────────────┐
   │ runos bot run (long-poll Telegram → owner chat) │
   │   voice memo                                    │
...
**Package layout (`runos/`):**

| Module | Role |
|--------|------|
| `cli.py` | typer entrypoint; all subcommands (`sync`, `transform`, `analyze`, `journal`, `bot`, `setup`, `install-hourly-sync`, etc.) |
| `config.py` | pydantic-settings; runtime data dir = `~/.runos/` by default (configurable); derived paths for DB / tokens / reports / content / voice cache; `get_settings()` is intentionally NOT cached |
...
| `scheduler.py` | launchd plist render + install: daily (`com.runos.daily.plist`, deprecated post-simplification), hourly sync (`com.runos.hourly-sync.plist`), bot (`com.runos.telegram-bot.plist`) |

## Conventions (load-bearing — follow these)

**Storage**
- **Two-layer raw → structured.** Connectors write ONLY to `raw_response`. Anything in `activity`, `wellness_day`, `journal`, `bot_session`, `daily_summary` is derived. `runos rederive` rebuilds from raw with zero network. NEVER backfill structured tables from outside the transforms layer.
...
- **Date bucketing has ONE rule** in `runos/transforms/bucketing.py`. Strava's `start_date_local[:10]` is wall-clock — the trailing `Z` is FAKE, not UTC. Garmin's `calendarDate` is verbatim. Never re-project to UTC.
- **Migrations:** hand-written `.sql` files in `runos/migrations/`, numbered. Bump `runos/db.SCHEMA_VERSION`. Idempotent (already-applied skipped). No ORM, no Alembic.

**Analysis layer**
- **Read-only, pure stdlib, NO network.** `runos/analysis/*` modules NEVER call out. They read structured tables + user markdown trackers + journal, that's it. (A `socket`-blocking test in `tests/test_rederive.py` enforces this for `rederive`.)
...
- **Lenient parsers for user-maintained markdown:** missing file → `present=False`, malformed lines skipped, unknown keys ignored, NEVER raise. See `runos/analysis/races.py::parse_races` as the model. Weight + nutrition parsers also expose `malformed_lines`; older parsers (races, heat, strength) do not yet — known asymmetry.
- **Reports degrade gracefully:** every report header states per-source data freshness (`runos/analysis/report.py`). Insufficient data → flag "insufficient", never invent. Empty/missing sections are omitted entirely, not rendered with "N/A".
...
- **Subjective rows are written ONLY via validated boundaries.** `runos.journal.service.add_entry` for journal entries. No free-form SQL paths for Claude. `runos journal add` CLI is a thin wrapper.
...
- **Tokens stored atomically:** temp-write → fsync → rename, mode 0600. A crash mid-write never leaves a corrupt token file. (See `runos/connectors/tokens.py`.)
- **`Connector` protocol** (`backfill(raw)` / `sync(raw, since)`) is the shape both Strava and Garmin implement. Future connectors mirror it.

**CLI**
- **typer subcommand groups** for cohesive surfaces: `runos strava ...`, `runos garmin ...`, `runos journal ...`, `runos bot ...`, `runos analyze ...`.
- **No `runos db` direct-SQL surface.** All writes go through validated paths.

**Config + secrets**
- **`pydantic-settings` reading gitignored `.env`** at the repo root. `SecretStr` for token-like fields.
- **`validation_alias`** for bare env var names (`TELEGRAM_BOT_TOKEN`, `WHISPER_MODEL_NAME`, `VOICE_RETENTION_DAYS`, `RUNOS_TARGET_KCAL`) where matching external conventions matters; otherwise the `RUNOS_*` prefix applies.
...
- **Tests mirror modules:** `runos/x/y.py` → `tests/test_y.py`. Tests that exercise full pipelines (CLI, end-to-end) live in `tests/test_*_cli.py`.
- **Always run `uv run pytest tests/ -x --deselect tests/test_bot_transcribe.py::test_transcribe_file_real_fixture_returns_nonempty` + `uv run ruff check runos/ tests/` before commit.** ~680 tests currently; should always be green before merge.
...
**Files + dirs**
- All runtime data under `~/.runos/` by default (override via `RUNOS_DATA_DIR`). NEVER inside the repo tree.
- Content dir (markdown trackers — `races.md`, `heat.md`, `strength.md`, `weight.md`, `food.md`) defaults to `~/.runos/` but can be redirected via `RUNOS_CONTENT_DIR` to a more convenient project-local folder (e.g. the `training/` dir).
...
## How to use RunOS (the CLI surface)

```bash
# One-time setup
runos setup                             # interactive first-run wizard (recommended)
# OR manual:
runos init                              # create DB + schema
runos strava auth                       # OAuth handshake (browser)
runos garmin login                      # interactive, one-time (MFA prompt)
runos install-hourly-sync               # hourly Strava+Garmin sync, notify on failure
runos bot install-scheduler             # always-on Telegram bot (KeepAlive)

# Hourly (via launchd, automatic)
runos sync --notify-on-failure          # what the hourly LaunchAgent runs

# On-demand
runos sync                              # Strava + Garmin → raw (interactive, no notify)
runos transform                         # raw → structured
runos analyze recovery                  # one specific report
runos analyze                           # all reports
runos strava streams --prefer-with-hr --limit 200   # backfill HR streams for HR-recorded activities

# Subjective capture
runos journal add --rpe 6 --feel strong --notes "..." --day 2026-05-28 --sport Run
runos journal list
runos journal link-orphans              # post-transform hook also runs this automatically

# Telegram bot
runos bot run                           # foreground (testing); launchd runs this in background

# Maintenance
runos rederive                          # rebuild all structured tables from raw, zero network
runos bot purge-voice [--yes]           # wipe voice cache

# Deprecated (kept for compatibility, no longer scheduled)
runos run-daily                         # the OLD full pipeline (sync+transform+analyze); run on-demand only
runos install-scheduler                 # OLD daily plist; superseded by install-hourly-sync
```
...
- **SQLite cross-thread error.** `sqlite3.Connection` is bound to the thread that created it. If you open a connection in `asyncio.to_thread`, you must close it in the same `to_thread` call. (Was a bug in `runos/bot/app.py::_post_init`; fixed in the simplify pass.)
- **`claude-agent-sdk` 0.2.x message shapes.** `AssistantMessage` has NO `role` attribute; `TextBlock` has NO `.type` attribute (just `.text`). Detect by class name (`type(msg).__name__ == "AssistantMessage"`). (Fixed in `runos/bot/agent.py`.)
- **Telegram rejects empty messages** with `BadRequest("Message text is empty")`. Always guard reply text — Claude Code turns that end on a tool call produce empty assistant text. (`runos/bot/handlers.py::_run_agent_turn` substitutes `"(agent finished without a reply)"`.)
...
- **Strava stream lazy-fetch + selection bias.** `runos strava sync` doesn't pull streams by default. Of 1,814 HR-recorded activities only ~2 had streams pulled (the rest of the 733 streamed ones happened to be non-HR). Use `runos strava streams --prefer-with-hr --limit 200` to drain the HR-having queue rather than walking ascending.
