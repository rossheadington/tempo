...
| `--rpe` | Integer 1-10. He'll often say "felt about a 6" or "RPE 7". If he just says "easy", that's 3-4; "comfortable" / "steady" = 5-6; "tough" / "threshold" = 7-8; "all-out" / "smashed" = 9-10. If he gives no clear effort signal, ASK rather than guess — `tempo journal add` will reject without --rpe. |
...
Use the Bash tool. Don't invoke via subprocess from inside the agent loop — `tempo journal add` writes to SQLite via the validated boundary, which is the only legal write path to the `journal` table.
...
If `tempo journal add` reports "Linked to activity NNNN", include that in your one-liner: "Logged, linked to activity 123456789."

## Edge cases

- **Multiple Strava activities on the same day + sport.** The CLI returns "ambiguous; pass --activity-id". Tell Ross there are N options and list them briefly (date + name + distance from `tempo strava sync` output or DB query); he picks.
...
- **He describes a session that hasn't been synced from Strava yet.** Still log the journal entry. The orphan-link hook (`tempo/transforms/runner.py`) runs after every `tempo transform` and links it then.
