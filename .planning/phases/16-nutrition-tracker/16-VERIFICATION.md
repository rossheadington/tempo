...
| --------------------------------------- | -------------------------------------------------------------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `runos/analysis/nutrition.py`           | Frozen+slots dataclasses + `parse_food` + `daily_nutrition` + `nutrition_rollup` | VERIFIED   | 558 LoC. All 5 dataclasses `@dataclass(frozen=True, slots=True)`. Two-format parser via `_INLINE_RE` + `_BLOCK_HEADER_RE` + `_BLOCK_BULLET_RE`. Latest-wins dedup. Left-open-right-closed windows. |
| `runos/analysis/nutrition_report.py`    | Standalone-report renderer with header banner + 5 sections     | VERIFIED   | 192 LoC. `render_nutrition` + `_freshness_line` + 5 section renderers. Goal section guarded by `target_kcal is not None`. Absent-file short-circuit. |
...
| `Settings.target_kcal_default`          | Optional `int \| None`, `validation_alias="RUNOS_TARGET_KCAL"` | VERIFIED   | `config.py:131-144`. Default `None`; comment confirms unset → silently off.                                                                              |
| `runos analyze nutrition` CLI           | New command writing dated nutrition report                     | VERIFIED   | `cli.py:722-754`. Mirrors existing per-report commands; passes `food_path` + `target_kcal` + `reports_dir` to `runner.generate_nutrition`.              |
| `runos analyze` aggregate includes nutrition | Top-level analyze passes `food_path` + `target_kcal`         | VERIFIED   | `runner.generate_all` (`runner.py:440-486`) accepts both; emits nutrition report when `food_path is not None`. `cli.py:614-615` passes both at the aggregate call site. |
| `runos/sync/daily.py` threading         | Daily pipeline passes `food_path` + `target_kcal`              | VERIFIED   | `daily.py:111-112`. `runos run-daily` automatically renders nutrition.                                                                                  |
...
| `.env.example`                          | `RUNOS_TARGET_KCAL` documented + commented                     | VERIFIED   | `.env.example:131` carries the documented opt-in knob.                                                                                                  |
...
| `Settings.food_path`                      | `content_root / "food.md"` | `Settings.content_root` derived from `RUNOS_CONTENT_DIR` (Phase 14 wizard) | YES | FLOWING |
| `Settings.target_kcal_default`            | `target_kcal_default` | `RUNOS_TARGET_KCAL` env var via pydantic-settings | YES | FLOWING (when set; silent None otherwise) |
...
| Ruff lint clean                                 | `uv run ruff check runos/ tests/`                                                                       | `All checks passed!`                                  | PASS   |
...
| NUTR-04     | 16-01             | `parse_food` → `FoodContext`; `daily_nutrition(...)` → `DailyNutrition`; `nutrition_rollup(...)` → `NutritionRollup` (7d + optional target-delta)    | SATISFIED | All three functions ship in `runos/analysis/nutrition.py` with the LOCKED dataclass shapes; 22 unit tests cover happy paths + edge cases + rollup math.        |
| NUTR-05     | 16-02, 16-03      | `runos analyze nutrition` writes `reports/<date>-nutrition.md`; recovery report adds `## Nutrition` section with 3-state degradation (absent/stale/current) | SATISFIED | `cli.py::analyze_nutrition` registered; `runner.generate_nutrition` writes dated file via `_write_report`; `_render_nutrition_section` enforces 3-state rule positioned after `## Weight`. |
...
None. `grep -n "TODO\|FIXME\|XXX\|TBD\|HACK\|PLACEHOLDER"` against `runos/analysis/nutrition.py` and `runos/analysis/nutrition_report.py` returns zero hits. No stubs, no empty handlers, no static-returning routes. Both modules are read-only / pure-stdlib / no-network — fully aligned with the project's analysis-layer convention.
...
1. **Parser + dataclasses (16-01)** — `runos/analysis/nutrition.py` with two-format lenient parser, latest-wins dedup, left-open-right-closed windows, optional kcal-goal threading via `Settings.target_kcal_default`. 22 unit tests, all passing.

2. **Standalone report (16-02)** — `runos analyze nutrition` CLI + `nutrition_report.render_nutrition` produce `reports/<date>-nutrition.md` with header banner + 5 sections (today's totals / per-meal breakdown / 7d rolling / 28d kcal / optional goal). 6 report tests.
...
4. **Docs + example (16-04)** — `food.md.example` (14 entries, both formats), `docs/NUTRITION.md` (394 lines), `.env.example` (RUNOS_TARGET_KCAL knob), `README.md` mention.

655 tests pass (Phase 15 was 619; +36 from this phase). Ruff clean. The example file is a live regression fixture. The nutrition rollup is wired through both the `runos run-daily` launchd path and the standalone `runos analyze nutrition` / `runos analyze recovery` / `runos analyze` commands. Optional goal-tracking activates silently when `RUNOS_TARGET_KCAL` is set.
