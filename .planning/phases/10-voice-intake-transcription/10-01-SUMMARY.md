...
plan: 01
subsystem: runos.bot.transcribe
tags: [voice, transcription, faster-whisper, settings, warmup]
status: complete
requires:
  - runos.bot.app (Phase 9 telegram bot scaffold)
  - runos.config.Settings (existing pydantic-settings shape)
provides:
  - runos.bot.transcribe.warm_model (idempotent WhisperModel singleton load)
  - runos.bot.transcribe.get_model (raises RuntimeError if unwarmed)
  - runos.bot.transcribe.transcribe_file (eager segments-generator consumer)
  ...
  - runos/bot/app.py::_post_init startup warmup (logs "Whisper model loaded and ready")
affects:
  - .env.example (new Whisper transcription section)
  - runos/bot/__init__.py (re-exports warm_model / get_model / transcribe_file)
tech-stack:
  added:
  ...
  patterns:
    ...
    - validation_alias bypass of RUNOS_ env_prefix (mirrors TELEGRAM_* pattern)
...
decisions:
  ...
  - voice_cache_dir is INTENTIONALLY kept out of ensure_dirs() so `runos init` does not surface a voice/ dir for users who never run the bot. Plan 10-02 will create it lazily 0700 in the voice handler.
...
Local-transcription substrate landed: `faster-whisper 1.2.1` is now a runtime dep, three new bare-env-var `WHISPER_*` Settings fields configure it (defaults: `small.en` / `int8` / `cpu`), and a module-level `WhisperModel` singleton in `runos/bot/transcribe.py` is warmed once via `asyncio.to_thread(warm_model, settings)` from the existing `post_init` hook so the first voice memo does not pay the multi-second model load.
...
3. Default `pytest` to `-m "not slow"` while keeping a `runos test --slow` recipe.
...
**418 passing** (411 after Task 1's `+5` config tests, then `+7` transcribe tests in Task 2). Baseline was 406 from Phase 9. Ruff check + format clean across `runos/` and `tests/`.

## Deviations from Plan

### Auto-fixed Issues

**1. [Setup - Worktree behind main]** The worktree branch `worktree-agent-a3fd8d04605b045c7` was created from commit `15c7103` (the v1.1 milestone start) which predates Phase 09's Telegram bot scaffold merging into `main`. This plan strictly depends on `runos/bot/app.py::_post_init`, `runos/bot/__init__.py`, and `runos/bot/handlers.py` (none of which existed at the worktree's HEAD). Fast-forward merged `main` (a clean fast-forward, no conflicts) into the worktree branch to bring the Phase 09 deliverables in.
...
| ---- | ------------ | ------- |
| Task 1 | 1 (ruff format auto-reflow of long lines in `runos/config.py`) | Applied via `ruff format`, all checks pass. |
| Task 2 | 1 (ruff format reflowed `runos/bot/transcribe.py` get_model error and `tests/test_bot_transcribe.py` signatures) | Applied via `ruff format`, all checks pass. |

## Threat Flags

None. This plan adds no new network endpoints, no new auth paths, and no new file-access surface beyond what the plan's `<threat_model>` (implicit via `voice_cache_dir`) already covers — the directory is gitignored via the existing `content_dir`/`~/.runos/` exclusions and creation is deferred to Plan 10-02 with explicit 0700 mode.
...
- [x] `runos/bot/transcribe.py` exists.
- [x] `tests/test_bot_transcribe.py` exists.
- [x] `tests/fixtures/voice/sample.ogg` exists (6.4 KB).
- [x] `runos/config.py` modified (whisper fields + voice_cache_dir property).
- [x] `runos/bot/app.py` modified (warm_model call in _post_init + settings stashed).
- [x] `runos/bot/__init__.py` re-exports warm_model / get_model / transcribe_file.
...
- [x] `uv run ruff check runos tests` + `ruff format --check` exit 0.
- [x] `grep "asyncio.to_thread(warm_model, settings)" runos/bot/app.py` matches.
- [x] `from runos.bot import warm_model, get_model, transcribe_file` succeeds.
