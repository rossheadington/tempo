...
provides:
  - "runos.analysis.context module deleted; runos.analysis.races is now the sole home for race parsing"
...
affects:
  - runos/analysis/runner.py (plan_path param + parse_plan call removed from generate_race_readiness + generate_all)
  - runos/analysis/report.py (plan_ctx param + plan-rendering block removed from render_race_readiness)
  - runos/analysis/race_link.py (docstring class ref now points at runos.analysis.races)
  - runos/cli.py (two plan_path kwarg sites removed from analyze + analyze race-readiness)
  - runos/sync/daily.py (one plan_path kwarg site removed from run_daily)
  - runos/config.py (Settings.plan_path property removed)
  - runos/analysis/__init__.py (module docstring scrubbed of plan.md / context shim mentions)
  ...
  - tests/test_noteworthy.py + tests/test_race_link.py (imports rewired to runos.analysis.races)
tech-stack:
  added: []
  patterns:
...
decisions:
  - "Reordered execution: ran Task 4 before Task 2 because Task 2's deletion of context.py would have broken the still-live `from runos.analysis.context import PlanContext, RacesContext` reference inside tests/test_analysis_reports.py::_render_with_links. Task 4 removed plan_ctx from render_race_readiness (and from that helper) first, then Task 2 deleted the shim. Final state is identical to the plan's stated end state; only the intermediate order changed."
  - "Scope expansion (Rule 3 — blocking issue): two additional `from runos.analysis.context import Race` import sites surfaced beyond RESEARCH.md section 7's enumerated 8 — runos/analysis/race_link.py:35 and tests/test_race_link.py:21. Migrated both in Task 1's commit."
  ...
  - "Marked TRACK-01, TRACK-02, TRACK-04 complete in REQUIREMENTS.md (and TRACK-06 per this plan). Verified TRACK-01 (Race.result field at races.py:46), TRACK-02 (RacesContext.completed at races.py:66), TRACK-04 (runos/analysis/heat.py + heat.md.example) are all satisfied by earlier Phase 8 plans but were not previously checked off."
metrics:
  duration: "9 min"
...
`plan.md` is fully retired — the parser, `PlanContext`, `parse_plan`, `_PLAN_FIELD_RE`, `Settings.plan_path`, `plan.md.example`, the `render_race_readiness(plan_ctx=...)` parameter, the `## Plan context` report section, and every README / `.env.example` / docstring mention are gone; `runos.analysis.races` is now the canonical home for race parsing with the transition shim from Plan 08-01 deleted; full test suite (398 tests) and ruff stay green.

## What changed

- **Import sweep (Task 1)** — 10 import sites total were rewired from `runos.analysis.context` to `runos.analysis.races`: the 8 enumerated in RESEARCH.md section 7 (`runos/analysis/runner.py:23,29`, `runos/analysis/report.py:20`, `runos/sync/daily.py:34`, `runos/analysis/__init__.py:16`, `tests/test_noteworthy.py:220,233`, and the test_context.py site which became moot when the file was deleted) plus 2 that PATTERNS.md and RESEARCH.md section 7 missed (`runos/analysis/race_link.py:35`, `tests/test_race_link.py:21`). All migrated to `from runos.analysis.races import Race`/`RacesContext`/`as ctx` patterns.
...
```
$ ls plan.md.example runos/analysis/context.py tests/test_context.py
ls: plan.md.example: No such file or directory
ls: runos/analysis/context.py: No such file or directory
...
```
$ grep -rn "plan_path\|PlanContext\|parse_plan\|_PLAN_FIELD_RE\|RUNOS_PLAN_PATH\|runos.analysis.context" \
    runos/ tests/ .env.example README.md
...
- **Files deleted:** 3 (`plan.md.example`, `runos/analysis/context.py`, `tests/test_context.py`)
- **Files modified:** 14 (5 in runos/, 6 in tests/, 2 docs, 1 config)
...
- File deletions: `plan.md.example`, `runos/analysis/context.py`, `tests/test_context.py` all confirmed absent.
...
- `uv run ruff check runos/ tests/` + `ruff format --check` → clean.
- `uv run python -c "from runos.analysis import races, heat, race_link, recovery, runner, report, data, load, fitness, race; print('all imports work')"` → exits 0.

## Deviations from Plan

### Reordered task execution: 1 → 3 → 4 → 2 → 5 (instead of plan's 1 → 2 → 3 → 4 → 5)

**Why:** Task 2's deletion of `runos/analysis/context.py` would have broken `tests/test_analysis_reports.py::_render_with_links`, which still imported `from runos.analysis.context import PlanContext, RacesContext` at the time. Task 4 was the one that removed `plan_ctx` from `render_race_readiness` (and from that helper), so it had to run BEFORE Task 2. Running Task 3 (config) first was order-independent and let me commit progress.
...
**1. [Rule 3 — blocking issue] Two additional `from runos.analysis.context import Race` import sites beyond RESEARCH.md section 7**
- **Found during:** Task 1 (final grep verification surfaced them)
- **Issue:** `runos/analysis/race_link.py:35` and `tests/test_race_link.py:21` both imported `Race` from the shim. They were not in RESEARCH.md section 7's enumerated list of 8.
- **Fix:** Migrated both to `from runos.analysis.races import Race`.
...
**3. [Rule 3 — blocking issue] Lingering `:class:~runos.analysis.context.Race` docstring reference in `race_link.py:5` after Task 2's deletion of context.py**
- **Found during:** Task 2's post-delete grep
- **Issue:** The module docstring still referred to the deleted `runos.analysis.context.Race` class.
- **Fix:** Rewrote as `:class:~runos.analysis.races.Race`.
...
1. **runos/analysis/race_link.py:35** — `from runos.analysis.context import Race`. Real production code; not enumerated in RESEARCH.md section 7.
2. **tests/test_race_link.py:21** — `from runos.analysis.context import Race`. Test code; not enumerated.
3. **runos/analysis/race_link.py:5** — docstring class reference `:class:~runos.analysis.context.Race`. Only surfaces after the file is deleted (grep then matches the docstring text).
4. **runos/analysis/runner.py:1-5** — module docstring referenced `:mod:context`. Updated in Task 4 to point at `races`/`heat`.
