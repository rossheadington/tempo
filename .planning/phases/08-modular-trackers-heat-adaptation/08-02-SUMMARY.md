...
requires: [runos/analysis/context.py (pattern source), runos/config.py.content_root]
provides:
  - runos.analysis.heat.parse_heat
  - runos.analysis.heat.heat_rollup
  - runos.analysis.heat.HeatSession
  - runos.analysis.heat.HeatContext
  - runos.analysis.heat.HeatRollup
  - Settings.heat_path
...
key_decisions:
  ...
  - "heat is not re-exported at the runos.analysis package level (import-on-demand convention; only journal symbols are re-exported)."
duration_min: 7
...
| File | Lines | Role |
| --- | --- | --- |
| `runos/analysis/heat.py` | 245 | new module: parser + HeatSession + HeatContext + heat_rollup + HeatRollup |
...
| `runos/config.py` | +5 | new `heat_path` property mirroring `races_path` |
| `tests/test_config.py` | +2 | parallel `heat_path` assertions inside the two existing content-dir tests |
| `runos/analysis/__init__.py` | +1 | module-index bullet for `heat` |
...
| `c522beb` | docs(08-02) | add heat module to runos.analysis package index |
...
- `uv run ruff check runos/analysis/heat.py runos/config.py runos/analysis/__init__.py tests/test_heat.py tests/test_config.py` → clean.
...
- **Found during:** Task 5 verification (`uv run ruff check runos/analysis/__init__.py`)
- **Issue:** The proposed bullet `* :mod:runos.analysis.heat -- parse heat.md + heat-adaptation session rollups for the recovery report (TRACK-04, TRACK-05).` rendered at 132 columns; ruff `E501` blocked the commit.
...
- **Files modified:** `runos/analysis/__init__.py`
...
- Pure stdlib, no network imports (`grep -c "import socket|import urllib|import requests|import httpx" runos/analysis/heat.py` = 0). ✓
...
- `runos/config.py` contains `heat_path` ✓
- `runos/analysis/__init__.py` contains `heat` bullet ✓
