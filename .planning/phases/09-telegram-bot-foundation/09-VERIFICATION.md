...
- **G1 ✓** — `tempo bot run` exists as a typer subcommand and calls into the bot worker.
  - Evidence: `tempo/cli.py:357-361` declares `bot_app = typer.Typer(...)` and `app.add_typer(bot_app, name="bot")`. `tempo/cli.py:364-381` defines `@bot_app.command("run") def bot_run_cmd()` which lazy-imports `tempo.bot.run` and invokes it, catching `ValueError` -> red stderr + `typer.Exit(1)`.
  - Smoke: `uv run tempo --help` lists `bot`; `uv run tempo bot --help` lists `run` with its docstring.

- **G2 ✓** — Chat-id allowlist enforced at filter level (non-owner messages silently dropped — no handler invocation, no reply).
  - Evidence: `tempo/bot/app.py:92-93` registers the handler as `CommandHandler("start", start_handler, filters=filters.Chat(chat_id=owner_chat_id))`. `tests/test_bot_app.py:174-193` (`test_start_command_filter_drops_non_owner`) asserts `handler.check_update(non_owner_update)` is falsy and `handler.check_update(owner_update)` is truthy — proving the dispatcher drops non-owner before any handler runs. Defence-in-depth in-handler re-check at `tempo/bot/handlers.py:40-49`.
...
- **G4 ✓** — Both env vars loaded via pydantic-settings with `validation_alias` (bare names, no `TEMPO_` prefix).
  - Evidence: `tempo/config.py:83-99` declares both fields with `validation_alias="TELEGRAM_BOT_TOKEN"` and `validation_alias="TELEGRAM_OWNER_CHAT_ID"`, bypassing the class-wide `env_prefix="TEMPO_"`. Test `test_settings_load_with_both_vars_set` confirms the bare names work (chat id is `int`, token is `SecretStr` scrubbed from `repr`).

- **G5 ✓** — Missing env var produces a clear startup error (no cryptic stack trace).
  - Evidence: `tempo/bot/app.py:26-42` `_require_telegram_config` raises `ValueError("Set TELEGRAM_BOT_TOKEN and TELEGRAM_OWNER_CHAT_ID in your .env -- see docs/TELEGRAM_BOT.md")` when either field is `None`. CLI wrapper in `tempo/cli.py:377-381` catches it and prints to red stderr + exit code 1.
  - Smoke (run with truly unset env): `env -i HOME=$HOME PATH=$PATH uv run --project /Users/rossheadington/Projects/tempo tempo bot run` -> exit 1, output: `Set TELEGRAM_BOT_TOKEN and TELEGRAM_OWNER_CHAT_ID in your .env -- see docs/TELEGRAM_BOT.md`. Tests 3 & 4 prove this path.
...
- **SC5 (structured logging to stdout, sleep/wake graceful, no webhook conflict) ✓** — `tempo/bot/app.py:45-57` `_configure_logging` calls `basicConfig(level=INFO, stream=sys.stdout)` and demotes httpx/httpcore to WARNING. PTB's default `stop_signals=(SIGINT, SIGTERM, SIGABRT)` left intact. `_post_init` hook at lines 82-85 calls `delete_webhook(drop_pending_updates=False)` to dodge the 409 Conflict pitfall while preserving offline messages.

## Requirement coverage

- **VOICE-01 ✓** — Single-chat allowlist with silent drop for non-owner. Implemented at the `filters.Chat(chat_id=...)` registration level (`tempo/bot/app.py:92-93`) with defensive in-handler re-check (`tempo/bot/handlers.py:40-49`). Tests prove both the filter-level drop and the defensive check.
- **VOICE-02 ✓** — Token + chat-id from `.env` via pydantic-settings; clear startup error if missing; `.env.example` documents both. Implemented via `validation_alias` in `tempo/config.py:83-99`, error in `_require_telegram_config` (`tempo/bot/app.py:26-42`), documented in `.env.example:70-75` and `docs/TELEGRAM_BOT.md`.
...
- **ruff**: `uv run ruff check tempo/ tests/` -> `All checks passed!`.
- **CLI smoke**:
  - `uv run tempo --help | grep -i bot` -> `│ bot                Telegram bot (v1.1): owner-only long-polling worker. │`.
  - `uv run tempo bot --help` -> lists `run` subcommand.
  - `env -i HOME=$HOME PATH=$PATH uv run tempo bot run` (truly unset env) -> exits 1 with `Set TELEGRAM_BOT_TOKEN and TELEGRAM_OWNER_CHAT_ID in your .env -- see docs/TELEGRAM_BOT.md`.
- **Doc grep**:
...
|----------|--------|------------|-------|-----------|--------|
| `tempo/bot/__init__.py` | ✓ | ✓ (29 lines, exports `GREETING`, `build_application`, `run`, `start_handler`) | ✓ (imported by tests + cli lazy import) | n/a | VERIFIED |
| `tempo/bot/app.py` | ✓ | ✓ (`_require_telegram_config`, `_configure_logging`, `build_application`, `run`) | ✓ (used by `tempo/cli.py:375`) | n/a | VERIFIED |
| `tempo/bot/handlers.py` | ✓ | ✓ (`GREETING`, `start_handler` with HTML parse_mode) | ✓ (registered in app.py:93) | n/a | VERIFIED |
| `tempo/config.py` (`telegram_bot_token`, `telegram_owner_chat_id`) | ✓ | ✓ (both fields with `validation_alias`, `SecretStr` + `int`) | ✓ (read by `_require_telegram_config`) | ✓ (Settings -> bot.run -> ApplicationBuilder) | VERIFIED |
| `tempo/cli.py::bot_app` + `bot_run_cmd` | ✓ | ✓ (typer group + `run` command with ValueError->exit(1)) | ✓ (`app.add_typer(bot_app, name="bot")`) | n/a | VERIFIED |
...
| From | To | Via | Status |
|------|-----|-----|--------|
| `tempo bot run` CLI | `tempo.bot.run` | Lazy import `from tempo.bot import run as bot_run` (`tempo/cli.py:375`) | WIRED |
| `tempo.bot.run` | `tempo.config.get_settings` | direct call (`tempo/bot/app.py:111`) | WIRED |
| `build_application` | `_require_telegram_config` (config -> ValueError) | direct call (`tempo/bot/app.py:80`) | WIRED |
| `build_application` | `filters.Chat(chat_id=owner_chat_id)` | `CommandHandler("start", ..., filters=owner_filter)` (`tempo/bot/app.py:92-93`) | WIRED |
| `start_handler` reply | `update.message.reply_text(GREETING, parse_mode=ParseMode.HTML)` | `tempo/bot/handlers.py:54` | WIRED |
...
|------|---------|----------|--------|
| (none in modified files) | No `TODO/FIXME/XXX/HACK/PLACEHOLDER` debt markers found in `tempo/bot/`, `tempo/cli.py:bot_*`, `tempo/config.py` Telegram block, `tests/test_bot_app.py`, `docs/TELEGRAM_BOT.md`. | — | — |
