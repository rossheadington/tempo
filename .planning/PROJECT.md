...
RunOS is a personal, local-first training and health system for a runner. It
...
- **v1.3** (Phase 14): First-run setup wizard (`runos setup`).
- **v1.4** (Phase 15): Weight tracker (`weight.md`) with EWMA trend, kg/lb normalisation.
- **v1.5** (Phase 16): Nutrition tracker (`food.md`) with two-format parser, standalone `runos analyze nutrition` report.
...
- Operational model simplification: dropped the daily-report schedule; introduced hourly `runos sync --notify-on-failure --with-recent-streams` via launchd `StartInterval`; daily reports now on-demand via the bot.
- `--prefer-with-hr` flag on `runos strava streams` for targeted HR-stream backfills.
...
- **Pi port** — long-standing v1.2 deferral. systemd timer + service for the hourly sync, ARM `curl_cffi` wheel work, `runos install-hourly-sync --systemd` variant. `docs/RASPBERRY_PI.md` already captures the host details.
...
- **Backup story** — nightly encrypted dump of `~/.runos/runos.db` + `training/*.md` to a cloud target. Only the journal/heat/strength/weight/food data is irreplaceable; Strava+Garmin can be re-pulled within their retention windows.
...
**Plan & reflect**
- [ ] Maintain upcoming races (date, distance, goal) in a simple markdown file RunOS reads for context
- [ ] Maintain a training plan in a simple markdown file RunOS reads for context
...
- **Goals span four areas:** recovery/overtraining, training load & trends, race
  ...
  journaling.
- **Repo:** https://github.com/rossheadington/RunOS (public, code-only; all
...
- **Plan model:** training plan and race calendar are simple markdown files the
  user maintains; RunOS reads them for analysis context rather than diffing
...
|----------|-----------|---------|
| Name: RunOS | Running term (tempo runs) + the steady rhythm of scheduled data collection | — Pending |
