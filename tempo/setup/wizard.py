"""First-run setup wizard orchestrator (``tempo setup``).
...
* Every credentialed step **delegates** to an existing helper: DB →
  :func:`tempo.cli._init`; Strava → :func:`tempo.connectors.factory.build_strava_connector`
  + ``connector.authorization_url`` + ``connector.exchange_code`` (the same
  three calls ``tempo strava auth`` makes); Garmin →
  :func:`tempo.connectors.factory.garmin_login`; daily scheduler →
  :func:`tempo.scheduler.install_plist`; bot scheduler →
  :func:`tempo.scheduler.install_telegram_bot_plist`; smoke →
  :func:`tempo.sync.pipeline.run_full_sync`. No duplicated handshakes, no
  duplicated plist renders, no duplicated MFA prompt.
* Every ``.env`` write goes through :func:`tempo.setup.env_io.atomic_write_env`.
  Credentials are written **before** the downstream call so a partial failure
  leaves creds in place for retry.
* Every state check goes through :func:`tempo.setup.state.detect_install_state`.
...
from tempo.setup.env_io import atomic_write_env, read_env
from tempo.setup.prompts import (
    confirm_yn,
    print_block,
    print_indicator,
    print_step_banner,
...
def step_welcome(
    ...
    *,
    ...
    print_step_banner("welcome", "Tempo first-run setup")
    typer.echo(
        "Walks you from zero (no DB, no .env, no tokens) to a working `tempo run-daily`."
    )

    if sys.version_info < (3, 14):  # noqa: UP036 - runtime guard for misconfigured installs
        v = sys.version_info
        typer.secho(
            f"Tempo requires Python 3.14+ (found {v.major}.{v.minor}).",
            fg=typer.colors.RED,
            err=True,
        )
    ...
    if shutil.which("uv") is None:
        typer.secho(
            "Warning: `uv` not found on PATH. That's OK if you run with `python -m tempo`.",
...
def step_content(
    ...
    *,
    ...
    existing = env.get("TEMPO_CONTENT_DIR")
    if existing:
        print_indicator(f"TEMPO_CONTENT_DIR={existing}", "set")
    ...
    _check_non_interactive(non_interactive, "TEMPO_CONTENT_DIR")
    default = str(Path.home() / ".tempo")
    # Detect project-local option: only relevant when run from the Tempo repo.
    project_local = Path.cwd() / "training"
    if (Path.cwd() / "tempo" / "cli.py").exists():
    ...
    atomic_write_env(_ENV_PATH, {"TEMPO_CONTENT_DIR": str(chosen)})
    print_indicator(f"TEMPO_CONTENT_DIR={chosen}", "fresh")
...
def step_strava(
    ...
    *,
    ...
    atomic_write_env(
        _ENV_PATH,
        {
            "TEMPO_STRAVA_CLIENT_ID": client_id,
            "TEMPO_STRAVA_CLIENT_SECRET": client_secret,
        },
    )
    # Re-read settings so build_strava_connector sees the new env keys.
    settings = get_settings()

    from tempo.connectors.factory import build_strava_connector
    ...
    try:
        ...
        typer.echo("Re-run with `tempo setup --only=strava` to retry.")
        return StepResult("strava", "failed", str(exc))
...
def step_garmin(
    ...
    *,
    ...
    atomic_write_env(
        _ENV_PATH,
        {"TEMPO_GARMIN_EMAIL": email, "TEMPO_GARMIN_PASSWORD": password},
    )
    settings = get_settings()
...
def step_telegram(
    ...
    *,
    ...
    # Note: bare names (NOT TEMPO_-prefixed) -- matches .env.example convention.
    atomic_write_env(
        _ENV_PATH,
...
def step_scheduler(
    ...
    *,
    ...
    if state.daily_scheduler_installed and (not only or "scheduler" not in only):
        print_indicator("~/Library/LaunchAgents/com.tempo.daily.plist", "done")
...
def step_bot_scheduler(
    ...
    *,
    ...
    if state.bot_scheduler_installed and (not only or "bot-scheduler" not in only):
        print_indicator("~/Library/LaunchAgents/com.tempo.telegram-bot.plist", "done")
...
def step_smoke(
    ...
    *,
    ...
    print_step_banner("smoke", "Smoke test: tempo sync")
    ...
    if strava_terminal_failure:
        typer.secho(
            "Strava sync failed terminally. Re-run with `tempo setup --only=strava`.",
            fg=typer.colors.RED,
        )
...
def step_finish(
    ...
    typer.echo("\nNext steps:")
    typer.echo("  - View today's report: `ls ~/.tempo/reports/`")
    if state_after.daily_scheduler_installed:
        typer.echo(
            "  - Enable the daily job: `launchctl bootstrap gui/$UID "
            "~/Library/LaunchAgents/com.tempo.daily.plist`"
        )
