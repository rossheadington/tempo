"""Parse the user-maintained ``races.md`` and ``plan.md`` context files.

These are simple, low-friction markdown files the user maintains by hand; Tempo
reads them for analysis context (PLAN-01/02) -- it does NOT diff planned-vs-actual
(that is a deliberate anti-feature; the plan is read for qualitative context only).
...
Documented format (see the committed ``races.md.example`` / ``plan.md.example``):
...
``plan.md`` -- free-form; we capture the whole text plus any ``## heading`` blocks
and any ``Phase:`` / ``Week:`` / ``Focus:`` lines as light structure, so the report
can quote the current focus without imposing a schema.
"""
...
from tempo.analysis.race import STANDARD_DISTANCES_M
...
def _parse_race_line(line: str) -> Race | None:
    ...
    # `date:`/`distance:`/`goal:`/`priority:` pairs) for races.
...
def parse_plan(path: Path) -> PlanContext:
    ...
    for line in text.splitlines():
        ...
        if m:
            ...
            # First occurrence wins (the current/top-of-file focus).
