"""Claude Agent SDK wrapper for the Tempo Telegram bot (Phase 11 / VOICE-07/09/13).

This module is the **only** seam between Tempo and ``claude_agent_sdk``: Plan
...
handler tests can ``monkeypatch.setattr("tempo.bot.agent.run_turn", ...)``
...
* :func:`run_turn` -- the async entry point. Builds
  :class:`ClaudeAgentOptions` with the Tempo project root as ``cwd`` (so the
  agent has access to Tempo's slash commands, skills, and gitignored data
...
class AgentTurn:
    ...
    Attributes:
        ...
        session_id: Session id surfaced on the final ``ResultMessage``.
            ...
            persist via :func:`tempo.bot.sessions.save_session`.
    ...
    *,
    ...
    Args:
        ...
        session_id: Existing Claude Code session id to resume, or
            ...
            :func:`tempo.bot.sessions.get_or_create_session`.
        cwd: Working directory the agent runs in. Plan 11-03 passes the
            Tempo repo root so the agent has access to Tempo's slash
