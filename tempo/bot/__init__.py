"""Telegram bot scaffold (Phase 9 / v1.1) + voice transcription (Phase 10) + sessions (Phase 11).
...
Code session-id store (Plan 11-01) and the Claude Agent SDK wrapper
(Plan 11-02) ahead of the handler wiring in Plan 11-03. The package is
import-safe: importing :mod:`tempo.bot` never starts the bot, never touches
the network, never downloads a Whisper model, never spawns the ``claude``
CLI, and never calls :func:`logging.basicConfig` -- the model is only loaded
when :func:`warm_model` is called from :func:`tempo.bot.app._post_init` at
startup, and the SDK is only invoked when :func:`run_turn` is called from a
handler.
