...
Drives the commands as a user would (via the temp RUNOS_DATA_DIR), with the
...
@pytest.fixture
def seeded_data_dir(runos_data_dir: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
...
def test_install_scheduler_writes_template(runos_data_dir: Path) -> None:
    ...
    plist = runos_data_dir / "launchd" / "com.runos.daily.plist"
    assert plist.exists()
...
def test_install_scheduler_does_not_touch_launch_agents(runos_data_dir: Path) -> None:
    ...
    assert str(runos_data_dir / "launchd") in result.output
    assert str(scheduler.launch_agents_dir()) not in result.output.split("To enable")[0]


# ---------------------------------------------------------------------------
# Phase 12: `runos bot install-scheduler` (long-running KeepAlive plist)
# ---------------------------------------------------------------------------
...
def test_bot_install_scheduler_writes_template_and_creates_logs(
    ...
    """`runos bot install-scheduler` writes a valid plist under the project's
    ...
    assert "launchctl start com.runos.telegram-bot" in result.output
    assert "never runs launchctl for you" in result.output.lower()

    plist = project_root / "launchd" / "com.runos.telegram-bot.plist"
    assert plist.exists()
    parsed = plistlib.loads(plist.read_text().encode())
...
# ---------------------------------------------------------------------------
# Phase 12: `runos bot purge-voice` (manual privacy hatch)
# ---------------------------------------------------------------------------


def test_bot_purge_voice_with_yes_deletes_all_files(runos_data_dir: Path) -> None:
    """`runos bot purge-voice --yes` deletes every file in voice_cache_dir."""
    settings = get_settings()
...
def test_bot_purge_voice_no_files(runos_data_dir: Path) -> None:
...
def test_bot_purge_voice_no_dir(runos_data_dir: Path) -> None:
...
def test_bot_purge_voice_without_yes_prompts_and_aborts_on_no(runos_data_dir: Path) -> None:
...
def test_bot_purge_voice_without_yes_proceeds_on_yes(runos_data_dir: Path) -> None:
