...
The hourly ``runos sync --notify-on-failure`` job uses this to send the owner a
...
* :func:`format_failure_message` -- compose the HTML body from a list of
  :class:`runos.sync.pipeline.SourceResult`.
...
def format_exception_message(exc: BaseException) -> str:
    ...
    return f"<b>❌ RunOS sync crashed</b>\n<code>{cls}</code>: {msg}"
