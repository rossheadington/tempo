...
Implements the :class:`~runos.connectors.base.Connector` protocol for Garmin
Connect via the *unofficial* ``garminconnect`` library. Garmin is the single most
fragile dependency in RunOS (no official personal API; reverse-engineered SSO
...
The interactive ``runos garmin login`` is the *only* path that submits the
...
itself raises on failure, but the *caller* (the ``runos sync`` pipeline, see
:func:`runos.sync.pipeline.run_garmin_sync`) wraps Garmin in a try/except so a
...
:mod:`runos.transforms.wellness`, a pure no-network pass.
"""
...
class GarminAuthError(RuntimeError):
    ...
    one-time ``runos garmin login``" (no persisted tokens, or login rejected),
...
class GarminConnector:
    ...
    def _authenticated_client(self) -> GarminClient:
        ...
        run ``runos garmin login`` -- it never retries or prompts.
        """
        ...
        try:
            ...
            raise GarminAuthError(
                "No persisted Garmin tokens found. Run `runos garmin login` once first."
            ) from exc
            ...
            raise GarminAuthError(
                ...
                "Run `runos garmin login` to re-authenticate."
            ) from exc
        self._client = client
        return client
