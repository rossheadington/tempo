...
Implements the :class:`~runos.connectors.base.Connector` protocol for Strava's
...
(rotating) refresh token is written via :class:`~runos.connectors.tokens.TokenStore`
...
class StravaConnector:
    ...
    def stored_activity_ids(
        ...
        structured layer to be populated (``runos transform`` has been run).
