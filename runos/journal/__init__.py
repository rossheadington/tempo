...
validated :func:`runos.journal.service.add_entry` entrypoint -- never by writing
...
* resolves the activity by local date + sport (0 / 1 / many matches handled
  explicitly, see :func:`runos.journal.service.resolve_activity`);
...
The CLI command ``runos journal add`` is a thin wrapper over :func:`add_entry`.
"""

from runos.journal.service import (
    ActivityMatch,
    JournalEntry,
    JournalError,
