"""The ``runos`` command-line interface — Typer entrypoint for every subcommand.
...
* ``runos bot ...``                     -- Telegram bot run + scheduler +
                                           voice-cache purge.
* ``runos sync`` / ``transform`` /
  ``rederive`` / ``analyze`` /
  ``run-daily``                         -- the core daily pipeline.
* ``runos install-scheduler``           -- launchd plist for the daily run.
...
Running bare ``runos`` (also ``runos init``) creates the runtime data dir,
opens/creates the SQLite DB in WAL mode, and applies migrations so every
table exists for the schema version recorded in ``runos.db.SCHEMA_VERSION``.
"""
...
from runos.connectors.factory import build_strava_connector, strava_token_store
...
def _init() -> None:
    ...
    typer.echo(f"RunOS {__version__}")
...
def main(ctx: typer.Context) -> None:
    """Run ``runos`` with no subcommand to initialise the database."""
...
def strava_auth(
    ...
    if code is None:
        ...
        typer.echo(url)
        typer.echo("\nThen run:  runos strava auth --code <CODE>")
        return
...
def strava_backfill(
    ...
    ``runos strava streams`` to fetch those lazily.
    """
...
def strava_streams(
    ...
    prefer_with_hr: bool = typer.Option(
        ...
        "most-recent first. Requires `runos transform` has been run.",
    ),
) -> None:
...
def sync(
    ...
    a later ``runos analyze`` still complete on existing data (GRMN-01/03).
    ...
    """
    from runos.sync import notify, pipeline
    ...
    try:
        try:
            ...
            # Strava credentials missing -- the same UX as `runos strava sync`.
    ...
    for r in results:
        if r.ok:
            ...
            # A skipped Garmin run is NOT a failure of `runos sync` -- surfaced
...
def garmin_login_cmd() -> None:
    ...
    tokens under the tokens dir, so every later ``runos sync`` reuses them and
    NEVER logs in again -- which is what keeps you clear of Garmin's per-account
    429 lockout. Set RUNOS_GARMIN_EMAIL / RUNOS_GARMIN_PASSWORD in your .env first.
    """
...
def garmin_backfill_cmd(
    ...
    Reuses persisted tokens (run ``runos garmin login`` first). On a 429 it stops
    immediately with NO retry -- resume later. Isolated: a failure here exits
    non-zero but never corrupts the Strava data.
    """
    from runos.connectors.factory import build_garmin_connector
    from runos.connectors.garmin import SOURCE as GARMIN_SOURCE
    from runos.connectors.garmin import GarminAuthError, GarminSyncError
...
def bot_install_scheduler_cmd(
    ...
    """Generate a long-running launchd LaunchAgent plist for `runos bot run`.

    Renders the committed launchd/com.runos.telegram-bot.plist TEMPLATE,
    ...
    RunOS NEVER runs ``launchctl`` for you -- loading is an explicit,
    ...
    typer.echo(
        ...
        "(KeepAlive=true). RunOS never runs launchctl for you."
    )
...
def bot_run_cmd() -> None:
    ...
    """
    # Lazy import so `runos --help` on a machine without python-telegram-bot
    # installed doesn't blow up at module load. Mirrors the lazy-import
    # pattern used by `sync()` above.
    from runos.bot import run as bot_run
...
def _analyze_setup():
    ...
    """
    from runos.analysis import load as load_mod
    from runos.analysis.preferences import PreferencesContext, parse_preferences
    from runos.analysis.runner import _load_config_from_prefs
...
def journal_main(ctx: typer.Context) -> None:
    ...
    if ctx.invoked_subcommand is None:
        typer.echo("runos journal: capture subjective entries.")
        typer.echo(
            "  runos journal add  --rpe 7 --feel strong --notes '...' "
            "[--day --sport --activity-id --duration-min]"
        )
        typer.echo("  runos journal list [--limit N]")
...
def journal_add(
    ...
    from runos.journal import JournalError, add_entry
...
def journal_link_orphans(
    ...
    Auto-runs after every ``runos transform`` as a post-transform hook -- this
    ...
    """
    from runos.journal import link_orphan_entries
...
def journal_list(
    ...
    from runos.journal import list_entries
...
def install_scheduler_cmd(
    ...
    Renders a plist that runs ``runos run-daily`` via ``StartCalendarInterval`` --
    ...
    typer.echo(
        ...
        "scheduler env still finds uv/python. RunOS never runs launchctl for you."
    )
...
def install_hourly_sync_cmd(
    ...
    The plist runs ``runos sync --notify-on-failure`` every ``--interval-s``
    ...
    installed the daily plist (``runos install-scheduler``), the printed
    bootout command below removes it cleanly.

    RunOS never runs ``launchctl`` itself -- the exact bootstrap/bootout
    ...
    daily_plist = Path.home() / "Library" / "LaunchAgents" / "com.runos.daily.plist"
...
def setup_cmd(
    ...
    skip_smoke: bool = typer.Option(
        False, "--skip-smoke", help="Skip the final `runos sync` smoke test."
    ),
