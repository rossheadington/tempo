...
* stashes ``settings.db_path`` in ``bot_data`` so handlers can open
  short-lived sqlite connections without re-importing :mod:`runos.config`;
* runs :func:`runos.db.init_db` in the ``post_init`` hook so migration 0005
  (the ``bot_session`` table) is guaranteed applied before the first handler
  fires, even on a fresh checkout without a prior ``runos init``;
...
from runos.bot.error_handler import telegram_error_handler
...
from runos.bot.transcribe import warm_model
...
logger = logging.getLogger("runos.bot")
...
def _require_telegram_config(settings: Settings) -> tuple[str, int]:
    ...
    existing pattern in :mod:`runos.connectors.factory`.
    """
...
def _configure_logging() -> None:
    ...
    INFO-level lines for runos + python-telegram-bot; httpx/httpcore demoted
...
def build_application(settings: Settings) -> Application:
    ...
    same hook also calls :func:`runos.db.init_db` to ensure migration 0005
    ...
    async def _post_init(application: Application) -> None:
        ...
        # where the user runs ``runos bot run`` without ``runos init`` first.
    ...
    # ``runos.db.connect(db_path)``. Stashing the path here means the
    # handler does not need to re-import ``runos.config``.
