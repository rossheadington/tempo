...
action (the README documents the exact ``launchctl`` commands). RunOS never loads
...
LABEL = "com.runos.daily"

# Phase 12: the long-running Telegram bot LaunchAgent label.
TELEGRAM_BOT_LABEL = "com.runos.telegram-bot"

# Hourly Strava+Garmin sync LaunchAgent label. Runs `runos sync
# --notify-on-failure` every hour via StartInterval; the daily-report job
# (`runos run-daily`) is intentionally NOT scheduled in this simplified
# operational model -- reports are generated on-demand via Telegram.
HOURLY_SYNC_LABEL = "com.runos.hourly-sync"
...
class SchedulerPaths:
    ...
    program: str  # absolute path to the launcher (uv or the runos entrypoint)
...
def _find_program(project_dir: Path, *subcommand: str) -> tuple[str, list[str]]:
    """Resolve how to invoke ``runos <subcommand>`` with absolute paths.

    Prefers ``uv run runos <subcommand>`` (matching how the project is run),
    falling back to the installed ``runos`` console script, then to
    ``python -m runos.cli <subcommand>``. All three are returned as
    ...
    uv = shutil.which("uv")
    if uv:
        return uv, [uv, "run", "runos", *cmd]
    runos_bin = shutil.which("runos")
    if runos_bin:
        return runos_bin, [runos_bin, *cmd]
    # Last resort: the current interpreter running the CLI module.
    return sys.executable, [sys.executable, "-m", "runos.cli", *cmd]
...
def render_plist(
    ...
    *,
    ...
    absolute ``PATH``, the project working dir, ``RUNOS_DATA_DIR`` so the scheduled
...
{arg_items}
    ...
    <!-- Absolute PATH + RUNOS_DATA_DIR so the stripped launchd env runs the job
         exactly like the interactive shell does (PITFALLS 7). -->
    <key>EnvironmentVariables</key>
    <dict>
        <key>PATH</key>
        <string>{escape(path_env)}</string>
        <key>RUNOS_DATA_DIR</key>
...
def install_plist(
    *,
    ...
    RunOS still does **not** run ``launchctl`` itself (that explicit step is left to
...
# ---------------------------------------------------------------------------
# Hourly sync LaunchAgent: `runos sync --notify-on-failure` every 3600s
# ---------------------------------------------------------------------------
...
def resolve_hourly_sync_paths(
    *, project_dir: Path, data_dir: Path
    ...
    The argv is ``runos sync --notify-on-failure --with-recent-streams`` so
    every hourly run:
...
def install_hourly_sync_plist(
    *,
    ...
    and points argv at ``runos sync --notify-on-failure`` instead of
    ``runos run-daily``. RunOS never runs ``launchctl`` itself -- the exact
...
TELEGRAM_BOT_TEMPLATE = (
    Path(__file__).resolve().parent.parent / "launchd" / "com.runos.telegram-bot.plist"
)
...
def render_telegram_bot_plist(
    *,
    ...
    The committed template (``launchd/com.runos.telegram-bot.plist``) is the
