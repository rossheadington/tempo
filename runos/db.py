...
``PRAGMA user_version``. Ordered ``NNNN_*.sql`` files in :mod:`runos.migrations`
...
def connect(db_path: Path) -> sqlite3.Connection:
    """Open a SQLite connection with RunOS's standard pragmas.
...
def _migration_files() -> list[tuple[int, str, str]]:
    ...
    files = resources.files("runos.migrations")
