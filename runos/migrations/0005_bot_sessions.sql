...
--                         All writes go through runos.bot.sessions; the schema
...
-- project convention from runos.journal.service. The 4-hour resume window is
-- evaluated in Python, not SQL, so the window is a code-side knob (see
-- runos.bot.sessions). Sessions persist indefinitely until ``/clear``.
