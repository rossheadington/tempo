...
This is a small read-only composition layer above :mod:`runos.analysis.data`
that implements the race-to-activity auto-link for TRACK-03. Given the list of
:class:`~runos.analysis.races.Race` objects parsed from the user's
...
The 0/1/N convention mirrors :mod:`runos.journal.service` (Phase 5) with one
...
def link_races_to_activities(races: list[Race], conn: sqlite3.Connection) -> list[RaceLink]:
    ...
    * 2+ -> ``unlinked_ambiguous`` (refuse to guess; mirrors journal-service 0/1/N).
    ...
    table-existence probe in :func:`runos.analysis.data.srpe_by_day`).
    """
    if not races:
        return []
