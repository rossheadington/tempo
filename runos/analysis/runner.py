...
This wires the read layer (:mod:`runos.analysis.data`), the pure metrics
(:mod:`load`, :mod:`fitness`, :mod:`race`), and the races/heat parsers
(:mod:`races`, :mod:`heat`) into the two reports the milestone ships:

* ``runos analyze load-trend``      -> ``reports/YYYY-MM-DD-load-trend.md``
* ``runos analyze race-readiness``  -> ``reports/YYYY-MM-DD-race-readiness.md``
* ``runos analyze`` (both)
...
from runos.analysis import correlation as corr_mod
from runos.analysis import data as dataread
from runos.analysis import fitness, load, race
...
def generate_load_trend(
    ...
    *,
    ...
    ``prefs.units`` to honour ``preferences.md`` (see :func:`runos.cli`).
    """
...
def generate_recovery(
    ...
    *,
    ...
    implemented in :func:`runos.analysis.recovery._render_heat_section`).
...
def generate_all(
    ...
    *,
    ...
    This is what the bare ``runos analyze`` and the daily scheduled run invoke. The
