...
return markdown text. The :mod:`runos.analysis.runner` gathers the findings and
writes the files.
"""
...
class WeeklyRollup:
    ...
    :func:`runos.units.format_distance` at the display boundary (Phase 17).
    """
    ...
    *,
    ...
    if not points:
        ...
        out.append(
            ...
            "transform Strava activities first (`runos sync && runos transform`).\n"
        )
        return "\n".join(out)
...
def render_race_readiness(
    *,
    ...
    if not races_ctx.present:
        out.append("## Races\n")
        out.append(
            "**No `races.md` found** -- create one (see `races.md.example`) so RunOS "
