...
- **atomic** — a crash or SIGINT mid-write must never leave a truncated/empty
  ``.env`` that would make RunOS unbootable;
...
The atomic-write template mirrors :class:`runos.connectors.tokens.TokenStore`
...
def atomic_write_env(
    ...
    :meth:`runos.connectors.tokens.TokenStore.save`: mkstemp in the destination
