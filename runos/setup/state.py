...
Answers a single question — *"what does this RunOS install look like right
...
- never shells out to ``launchctl`` (RunOS never runs ``launchctl`` per
...
def _db_initialised(settings: Settings) -> bool:
    ...
    A corrupt / non-RunOS SQLite file (or any DB-error) returns ``False`` so
...
def detect_install_state(settings: Settings) -> InstallState:
    ...
    """
    home = Path.home()
    daily_plist = home / "Library" / "LaunchAgents" / "com.runos.daily.plist"
    bot_plist = home / "Library" / "LaunchAgents" / "com.runos.telegram-bot.plist"
